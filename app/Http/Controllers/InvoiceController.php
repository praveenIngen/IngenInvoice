...
class InvoiceController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (\Auth::user()->can('manage invoice')) {
            ...
            $invoicesMonth = Invoice::selectRaw('invoices.*,MONTH(issue_date) as month,YEAR(issue_date) as year');

            // if ($request->status != '') {
            //     $invoicesMonth->where('status', $request->status);

            //     $filter['status'] = Invoice::$statues[$request->status];
            // } else {
            //     $invoicesMonth->where('status', '!=', 0);
            // }

            $invoicesMonth->where('created_by', '=', \Auth::user()->creatorId());
            ...
            $invoicesMonth->where('issue_date', '>=', date('Y-m-01', $start))->where('issue_date', '<=', date('Y-m-t', $end));

            $filter['startDateRange'] = date('M-Y', $start);
            $filter['endDateRange'] = date('M-Y', $end);

            if (!empty($request->customer)) {
                $invoicesMonth->where('customer_id', $request->customer);
                $cust = Customer::find($request->customer);

                $filter['customer'] = !empty($cust) ? $cust->name : '';
            }

            $invoicesMonth = $invoicesMonth->with(['customer', 'category'])->orderBy('id', 'DESC')->get();

            $totalInvoice = 0;
            $totalDueInvoice = 0;
            $invoiceTotalArray = [];
            foreach ($invoicesMonth as $invoiced) {
                $totalInvoice += $invoiced->getTotal();
                $totalDueInvoice += $invoiced->getDue();

                $invoiceTotalArray[$invoiced->month][] = $invoiced->getTotal();
            }
            $totalPaidInvoice = $totalInvoice - $totalDueInvoice;
// print_r($invoiceTotalArray);
// die;
            for ($i = 1; $i <= 12; $i++) {
            ...
            return view('invoice.index', compact('invoices', 'customer', 'status','invoiceTotal', 'monthList'));
        } else {
    ...
    public function create($customerId = 0)
    {
        if (\Auth::user()->can('create invoice')) {
            ...
            $product_services->prepend('Please Select Item', '');
            $url_pieces = explode( '?', $_SERVER['REQUEST_URI']);
            if($url_pieces[1]>0){
                $customerId=$url_pieces[1];
            }
...
//            $invoice->discount_apply = isset($request->discount_apply) ? 1 : 0;
            ...
            $totalPrice=0;
            $totalDiscount=0;
            $invoiceTypeDesc='STD';  //need to know
            $invoiceissue_date=$invoice->issue_date;
            $generateInvoiceData=Invoice::generateInvoiceThroughPotrtal($request,$invoice->invoice_id,$invoiceTypeDesc,$invoiceissue_date);
            if($generateInvoiceData->status=="SUCCESS"){
            $invoice->irn=$generateInvoiceData->fiscalisedInvoices[0]->irn;
           $invoice->qrCode=$generateInvoiceData->fiscalisedInvoices[0]->qrCode;
           $invoice->sales_transaction=$request->sales_transaction;
           $invoice->save();
              ...
              if(isset($products[$i]['item'])){
                ...
                // $invoiceProduct->discount    = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
                    ...
                    $invoiceNotificationArr = [
                        'invoice_number' => \Auth::user()->invoiceNumberFormat($invoice->id),
        ...
        //     die;
            ...
            //webhook
                ...
                return redirect()->route('invoice.index', $invoice->id)->with('success', __('Invoice successfully created.'));
            }elseif($generateInvoiceData->status == "ERROR"){
                $errorMeesage="";
                foreach($generateInvoiceData->fiscalisedInvoices[0]->errorMessages as $index => $error)
                {
                    if($index > 0){
                         $errorMeesage.=$error->description.",";
                    }else{
                         $errorMeesage=$error->description;
                    }
                }
                return redirect()->back()->with('error',$errorMeesage);
            }else{
                return redirect()->back()->with('error', __('Something went wrong.Kindly check Invoice Data'));
            }
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
            ...
            if ($invoice->created_by == \Auth::user()->creatorId()) {
                ...
                for ($i = 0; $i < count($products); $i++) {
                    ...
                    if (isset($products[$i]['item'])) {
                        ...
                        $invoiceProduct->price = $products[$i]['price'];
                        $invoiceProduct->description = $products[$i]['description'];
                        // $totalDiscount= $totalDiscount+$invoiceProduct->discount;
                        // $totaltaxPrice=  $totaltaxPrice+$invoiceProduct->tax;
                        // $totalPrice=  $totalPrice+$invoiceProduct->price+$totaltaxPrice;
                        $invoiceProduct->save();
                    }
                    if ($products[$i]['id'] > 0) {
                    ...
                    if (empty($products[$i]['id']) && isset($products[$i]['item'])) {
    ...
    public function show($ids)
    {
        ...
        if (\Auth::user()->can('show invoice')) {
            ...
            if (!empty($invoice->created_by) == \Auth::user()->creatorId()) {
                ...
                $seller=SellerDetail::getSellerDataforInvoice(\Auth::user()->name,\Auth::user()->creatorId());
                return view('invoice.view', compact('invoice', 'customer', 'seller','iteams', 'invoicePayment', 'customFields', 'user', 'invoice_user', 'user_plan' , 'creditnote'));
            } else {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        } else {
    ...
    public function print($ids)
    {
        ...
        if (!empty($invoice)) {
            ...
            $seller=SellerDetail::getSellerDataforInvoice(\Auth::user()->name,\Auth::user()->creatorId());
            return view('invoice.invoice_print', compact('settings', 'invoice','seller', 'customer', 'iteams', 'invoicePayment', 'customFields', 'user', 'company_payment_setting', 'user_plan'));
    ...
    public function sent($id)
    {
        ...
        if (\Auth::user()->can('send invoice')) {
            ...
            if ($setings['customer_invoice_sent'] == 1) {
                ...
                $customer = Customer::where('created_by', '=', \Auth::user()->creatorId())->where('id', $invoice->customer_id)->first();
...
//        dd($invoice_id);
    ...
    public function duplicate($invoice_id)
    {
        if (\Auth::user()->can('duplicate invoice')) {
            ...
            $invoiceTypeDesc='STD';  //need to know
            $invoice->duplicateCopy="Duplicate Copy";
            $invoiceissue_date=$duplicateInvoice->issue_date;
            $invoice->sales_transaction=$invoice->sales_transaction; 
            $duplicateInvoice->sales_transaction=$invoice->sales_transaction; 
            $generateInvoiceData=Invoice::generateInvoiceThroughPotrtal($invoice,$this->invoiceNumber(),$invoiceTypeDesc,$invoiceissue_date);
          
            if($generateInvoiceData->status=="SUCCESS"){
                $duplicateInvoice->irn=$generateInvoiceData->fiscalisedInvoices[0]->irn;
                $duplicateInvoice->qrCode=$generateInvoiceData->fiscalisedInvoices[0]->qrCode;
                // $duplicateInvoice->sales_transaction=$invoice->sales_transaction; 
    ...
    {
        try {
            $qrCode = Crypt::decrypt($invoiceId);
            $invoiceData = Invoice::where('qrCode', $qrCode)->first();
            $id=$invoiceData->id;
            if(empty($id)){
        ...
        // $invoice = Invoice::with(['invoice','payments.bankAccount','items.product.unit'])->find($id);
            // $id = Crypt::decrypt($invoiceId);
            $invoice = Invoice::with(['creditNote','payments.bankAccount','items.product.unit'])->find($id);
            //  $invoice = Invoice::find($id);
