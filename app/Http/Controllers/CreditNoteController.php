...
class CreditNoteController extends Controller
{
    ...
    public function store(Request $request, $invoice_id)
    {
        ...
        if(\Auth::user()->can('create credit note'))
        {
            ...
            $request->sales_transaction=$invoice->sales_transaction;
            $request->invoice_id=$invoice_id;
            $invoiceTypeDesc='CRN';  //need to know
            $invoiceissue_date=$invoice->issue_date;
            $generateInvoiceData=Invoice::generateInvoiceThroughPotrtal($request,$invoice_id,$invoiceTypeDesc,$invoiceissue_date);
            if($generateInvoiceData->status == "SUCCESS"){
                $credit->credit_irn=$generateInvoiceData->fiscalisedInvoices[0]->irn;
                $credit->credit_qrcode=$generateInvoiceData->fiscalisedInvoices[0]->qrCode;
                $credit->save();
                $customer->save();
    ...
    public function view($invoice_id, $creditNote_id)
    {
    ...
    public function update(Request $request, $invoice_id, $creditNote_id)
    {
        ...
        if(\Auth::user()->can('edit credit note'))
        {
            ...
            $credit->amount      = $request->amount;
            $credit->description = $request->description;
            $request->customer_id=$invoiceDue->customer_id;
            $request->sales_transaction=$invoiceDue->sales_transaction;
            $request->invoice_id=$invoice_id;
            $invoiceTypeDesc='CRN';  //need to know
            $invoiceissue_date=$invoiceDue->issue_date;
            $generateInvoiceData=Invoice::generateInvoiceThroughPotrtal($request,$invoice_id,$invoiceTypeDesc,$invoiceissue_date);
