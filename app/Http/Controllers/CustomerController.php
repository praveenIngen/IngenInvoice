...
class CustomerController extends Controller
{
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create customer'))
        {
//tax number is vat registration number
            $rules = [
                'name' => 'required',
                'contact' => 'required|digits:9',
                'tax_number' => 'digits:8|required|numeric',
                'business_registration_number' => 'required|alpha_num|max:10',
                'contact_number' => 'max:10',
                'nic_number'=>'required|min:14|max:14',
                'email' => [
                    'required',
            ...
            if($total_customer < $plan->max_customers || $plan->max_customers == -1)
            {
                ...
                $customer->transaction_nature           = $request->transaction_nature;
                $customer->nic_number           = $request->nic_number;
