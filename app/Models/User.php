...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    {
        // $settings = Utility::settings();
        $createdOn=DB::table('invoices')->select('created_at')->where('id','=',$number)->get();
        if(!empty($createdOn[0]->created_at)){
           $year=date("Y", strtotime($createdOn[0]->created_at));
           $invoice_prefix="INV-".$year."-";
        }else{
            $invoice_prefix="INV-".date("Y")."-";
        }
        return $invoice_prefix . sprintf("%05d", $number);
    }
    ...
    // {
    //     return Invoice::leftjoin('customers', 'invoices.created_by', '=', 'customers.created_by')->where('created_by', '=', \Auth::user()->creatorId())->count();
    //     //\DB::table('invoices')->where()->distinct('invoice_id')->count('invoice_id');
    // }
    ...
    {
        return $this->hasOne('App\Models\SellerDetail', 'user_id', 'id');
    }
